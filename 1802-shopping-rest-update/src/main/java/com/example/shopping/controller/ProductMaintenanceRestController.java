...
public class ProductMaintenanceRestController {
    ...
    @PostMapping
    public ResponseEntity<Void> registerProduct(@RequestBody ProductMaintenanceInput productMaintenanceInput) {
    	Product product = productMaintenanceService.register(productMaintenanceInput);
    	URI location = ServletUriComponentsBuilder.fromCurrentRequestUri().path("{id}").buildAndExpand(product.getId()).toUri();
