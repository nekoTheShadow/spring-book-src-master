...
public class SecurityConfig {
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		http
			...
			.requestMatchers(HttpMethod.POST, "/maintenance/product/**").hasAnyRole("MANAGER")
	...
	public UserDetailsService userDetailService() {
		UserDetails taro = User.builder().username("taro").password("{noop}taro123").roles("MANAGER").build();
		UserDetails jiro = User.builder().username("jiro").password("{noop}jiro123").roles("EMPLOYEE").build();
